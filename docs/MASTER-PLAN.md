...
**Nota 2026-04-26 (TASK-064/TASK-063 execution finale):** **TASK-064** resta **`BLOCKED`** dopo ciclo controllato Livello 1B: baseline locale A/B riallineata e outbox vuota prima dello smoke, ma S2 ha generato nuova outbox `PayloadValidation` su A pur propagando l'update a B. **TASK-063** resta **`BLOCKED`** in modalita `ACCEPTABLE` (OnePlus IN2013 + Medium Phone API 35, non `FULL`): S1 PASS, S2 solo parziale, S3-S6 non eseguiti. **TASK-060** resta **sospeso / `BLOCKED`** con evidenza UX positiva parziale da S2, non `DONE`. **TASK-055** resta **`PARTIAL`**. **TASK-062** e **TASK-061** restano **`DONE`**. Nessuna migration live, nessuna modifica DDL/RPC/RLS/publication Supabase remota autorizzata.
...
| Campo       | Valore                                                  |
|-------------|---------------------------------------------------------|
| Stato       | `PARTIAL` (audit/verifiche live e chiusura audit Supabase non completamente concluse; **TASK-059** `DONE`; **TASK-060** sospeso / `BLOCKED`; **TASK-061** `DONE`; **TASK-062** `DONE`; follow-up residui in **TASK-063** o decisione futura) |
| Priorità    | `ALTA`                                                  |
...
| Dipendenze  | **TASK-026** `DONE`, **TASK-044** `DONE`, **TASK-055** `PARTIAL`, **TASK-056** `PARTIAL` |
...
> **Contesto:** follow-up a **TASK-055** ( **`PARTIAL`** ). **TASK-059** è **`DONE`**. **TASK-060** è **sospeso / `BLOCKED`**. **TASK-061** è **`DONE`**. **TASK-062** è **`DONE`**. **TASK-063** è **`BLOCKED`**. **TASK-064** è **`BLOCKED`** — baseline pre-smoke verde, ma nuova outbox `PayloadValidation` su S2 live blocca B6/B9.
...
| Dipendenze | **TASK-055** `PARTIAL` (riferimento audit/UX); **TASK-057** / **TASK-058** `DONE` lato import/Database locale — nessun conflitto di stato |
...
| Campo | Valore |
|-------|--------|
| Stato | `BLOCKED` / sospeso (2026-04-26 — decisione esplicita utente; review non chiusa, non `DONE`) |
| Priorità | `ALTA` |
| Area | Supabase sync / DatabaseScreen / Paging / refresh UI |
| Dipendenze | **TASK-055** `PARTIAL`; **TASK-058** `DONE` (base refresh locale/scroll); **TASK-059** `DONE` |
...
| Descrizione | Quando un pull remoto applica modifiche a prodotti **già visibili** in `DatabaseScreen`, aggiornare **solo** le card interessate **senza** refresh globale della lista e **senza** far saltare lo scroll in cima. **Preferenza progettuale:** evento/ID locali toccati → `getProductDetailsById(id)` → aggiornamento `_productDetailsOverrides`; evitare `products.refresh()` globale come soluzione primaria. **Invariato:** Room fonte di verità; nessuna chiamata Supabase dai composable; nessun cambio architettura sync. **Esito utente:** se un altro device modifica un prodotto, la lista locale si aggiorna senza perdere la posizione. |
...
| Campo | Valore |
|-------|--------|
| Stato | **`BLOCKED`** (2026-04-26 — execution `ACCEPTABLE`: S1 PASS; S2 parziale per nuova outbox `PayloadValidation` su A pur con ricezione B senza scroll jump; S3-S6 bloccati; non `FULL`, non `DONE`) |
| Priorità | `ALTA` |
| Area | QA manuale / Supabase live / multi-device |
| File task | `docs/TASKS/TASK-063-smoke-live-a-b-sync-android-supabase.md` |
| Dipendenze | **TASK-055** `PARTIAL`; **TASK-059**/**TASK-061**/**TASK-062** `DONE`; **TASK-060** `BLOCKED` |
| Descrizione | Scenari minimi: device A e B stesso account Supabase; (1) A modifica prodotto/prezzo → B riceve update remoto **senza** scroll jump; (2) A aggiunge prodotto → B riceve; (3) A elimina con tombstone → B riceve; (4) offline → modifica locale → online → auto-push; (5) `sync_events` non disponibile/gap → UI propone full sync. Estensioni in file task: S7 RLS, S8 sessioni. **Nessun codice** salvo bug emersi e task fix separato. **Output richiesto:** matrice risultati con evidenze; elenco bug; follow-up tecnici. Runbook backend: `docs/SUPABASE.md`, `supabase/migrations/README.md`. |

### TASK-064 — Diagnosi outbox PayloadValidation e riallineamento baseline A/B
| Campo | Valore |
|-------|--------|
| Stato | **`BLOCKED`** (2026-04-26 — baseline controllata completata; outbox storica neutralizzata dal reset, ma S2 live ha riprodotto nuova `PayloadValidation` su A) |
| Priorità | `ALTA` |
...
| Dipendenze | **TASK-063** `BLOCKED` (sblocco smoke dopo diagnosi); **TASK-062**/**TASK-061** `DONE`; **TASK-055** `PARTIAL`; **TASK-060** `BLOCKED` |
| Descrizione | Piano sicuro per spiegare outbox pending con `PayloadValidation`, baseline Room A/B divergente, APK hash divergenti; prerequisiti per ripetere TASK-063. Esito finale: stesso APK/account, reset/bootstrap locale e outbox pre-smoke 0 su A/B; S2 live crea nuova outbox `PayloadValidation` su A mentre B riceve gli eventi. **Escluso e rispettato:** migration live, `supabase db push`, modifica DDL/RPC/RLS/publication, cleanup per nascondere il problema. |
