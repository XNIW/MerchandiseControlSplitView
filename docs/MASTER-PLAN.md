...
**Nota 2026-04-26 (TASK-064 init):** **TASK-064** e' l'**unico task attivo** in **`PLANNING`** — diagnosi outbox `PayloadValidation` e riallineamento baseline A/B prima di ripetere TASK-063; file `docs/TASKS/TASK-064-diagnosi-outbox-payloadvalidation-baseline-ab.md`. **TASK-063** resta **`BLOCKED`** (non `DONE`). **TASK-062** e **TASK-061** restano **`DONE`**. **TASK-060** resta **sospeso / `BLOCKED`**. **TASK-055** resta **`PARTIAL`**. Nessuna migration live, nessuna modifica DDL/RPC/RLS/publication da planning TASK-064.
...
| Campo               | Valore                                           |
|---------------------|--------------------------------------------------|
| Task attivo          | **TASK-064** — `PLANNING` (`docs/TASKS/TASK-064-diagnosi-outbox-payloadvalidation-baseline-ab.md`). **TASK-063** resta `BLOCKED`. **TASK-060** sospeso / `BLOCKED`. **TASK-055** `PARTIAL`. |
| Fase task attivo     | **`PLANNING`** — TASK-064 diagnosi outbox / baseline A-B; TASK-063 non riaperto. |
...
> **Contesto:** follow-up a **TASK-055** ( **`PARTIAL`** ). **TASK-059** è **`DONE`**. **TASK-060** è **sospeso / `BLOCKED`**. **TASK-061** è **`DONE`**. **TASK-062** è **`DONE`**. **TASK-063** è **`BLOCKED`**. **TASK-064** è **`PLANNING`** (attivo) — diagnosi outbox / baseline A-B.
...
| Campo | Valore |
|-------|--------|
| Stato | **`BLOCKED`** (2026-04-26 — execution `ACCEPTABLE` limitata: S1 non conforme; S2–S6 bloccati; non `FULL`, non `DONE`) |
| Priorità | `ALTA` |
...
| Campo | Valore |
|-------|--------|
| Stato | **`PLANNING`** (2026-04-26 — init planner; unico task attivo) |
| Priorità | `ALTA` |
...
| Descrizione | Piano sicuro per spiegare outbox pending con `PayloadValidation`, baseline Room A/B divergente, APK hash divergenti; prerequisiti per ripetere TASK-063. **Escluso in planning:** fix codice, migration live, `supabase db push`, cancellazione dati senza rollback, smoke A/B completo. |
