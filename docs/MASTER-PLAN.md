...
**Nota cross-repo 2026-05-13 (TASK-108 iOS parity):** su richiesta utente durante il FIX/PARITY di iOS TASK-108, Android e' stato usato come riferimento reale e poi allineato solo sulla superficie pubblica: la sezione cloud di `OptionsScreen` espone ora una singola CTA pubblica `Sincronizza ora` / `Sync now` / localizzazioni, collegata al full refresh gia' esistente che copre catalogo, prezzi e History/session. Il quick sync resta disponibile internamente per path automatici/event/retry, ma non e' piu' una seconda azione pubblica concorrente. File modificati: `OptionsScreen.kt`, `NavGraph.kt`, `values*/strings.xml`. Check PASS: `assembleDebug`, `testDebugUnitTest --tests '*CatalogSync*'`, `git diff --check`, install/launch smoke su OnePlus IN2013 con Options single-action visibile. Nessun task Android nuovo aperto; cambiamento tracciato come allineamento cross-repo a iOS TASK-108.

**Nota cross-repo 2026-05-14 (TASK-108 iOS local database status parity):** su richiesta utente nel FIX mirato post-TASK-108, Android ha adottato una card compatta `Local database status` in `OptionsScreen`, equivalente alla superficie iOS e senza introdurre una seconda CTA sync. La card mostra conteggi prodotti/fornitori/categorie/storico prezzi/History sessions, pending locali, ultimo sync/check e stato account quando disponibili; i conteggi arrivano da `CatalogSyncViewModel`/`InventoryRepository` su `Dispatchers.IO`. File modificati: `OptionsScreen.kt`, `CatalogSyncViewModel.kt`, `InventoryRepository.kt`, `ProductPriceDao.kt`, `NavGraph.kt`, `values*/strings.xml`, `RealtimeRefreshCoordinatorTest.kt`. Check PASS: `assembleDebug`, test mirati `*CatalogSync*` + `*RealtimeRefreshCoordinatorTest*`, `git diff --check`, install/launch smoke su OnePlus IN2013; UI hierarchy in locale cinese mostra `本地数据库状态`. Nessun task Android nuovo aperto; cambiamento tracciato come allineamento cross-repo a iOS TASK-108.

**Nota cross-repo 2026-05-14 (TASK-108 ProductPrice performance/page streaming):** su richiesta utente nel pass finale REVIEW+FIX performance di iOS TASK-108, Android e' stato auditato come riferimento sync reale e poi allineato dove risultava meno efficiente: `ProductPriceRemoteDataSource` espone ora `fetchProductPricesPage(afterId, limit)`, `SupabaseProductPriceRemoteDataSource` usa keyset `id > afterId` + `id ASC` + page size max `900`, e `InventoryRepository.pullProductPricesFromRemote` applica ProductPrice una pagina per volta invece di materializzare l'intera lista remota. Test aggiunto: `DefaultInventoryRepositoryTest` verifica `905` righe in 2 pagine (`900 + 5`) e `afterId` corretto. Check PASS: `assembleDebug`, test paging ProductPrice, `DefaultInventoryRepositoryTest`, `git diff --check`, install/launch su OnePlus IN2013 con PSS `151.044 KB` / RSS `233.876 KB`, logcat sensitive-token scan 0 match. `CatalogSyncViewModelTest` full class resta non eseguibile in questa shell per `AttachNotSupportedException`/MockK attach. Nessun task Android nuovo aperto; cambiamento tracciato come allineamento cross-repo a iOS TASK-108.

**Nota cross-repo 2026-05-14 (TASK-108 responsiveness verification):** nel FIX iOS TASK-108 su jank Options/MainActor, Android e' stato ricontrollato senza ulteriori patch di codice: il path ProductPrice resta paginato keyset, `InventoryRepository` lavora su `Dispatchers.IO`, `CatalogSyncViewModel` espone progress stage e `OptionsScreen` mantiene una sola CTA pubblica + `Local database status`. Check PASS: `assembleDebug` 14s, test mirato ProductPrice paging 8s, `git diff --check`, install/launch OnePlus IN2013, memory `dumpsys meminfo` TOTAL PSS `182.569 KB` / TOTAL RSS `281.960 KB`; nessun token/JWT/email raw aggiunto o loggato nel diff. Android app-auth live/cross-platform E2E resta non eseguito in questo pass e resta gap di TASK-108 iOS, non task Android separato.
...
**Avvio 2026-04-28 (TASK-070 execution):** **`TASK-070`** promosso a **`EXECUTION`** su richiesta utente — fix app-side per retry outbox head-of-line e logging strutturato privacy-safe; file `docs/TASKS/TASK-070-outbox-retry-head-of-line-logging-strutturato.md`. **TASK-069** confermato `DONE`; **TASK-071** resta backend separato e **non** è prerequisito. Vincoli confermati: nessun cleanup outbox, nessuna modifica UI, nessuna modifica Room schema/migration, Supabase, RPC, RLS, trigger o backend; preservare no-regression **TASK-068** bulk product push.

**Execution 2026-04-28 (TASK-070 → REVIEW):** **`TASK-070`** passa a **`REVIEW`** dopo patch app-side: `listPendingRetryable` aggiunta senza alterare `listPending`, retry outbox non bloccato da head FIFO a max attempts, outcome esplicito `NoOp`/`Recorded`/`Enqueued`/`PartiallyRecordedAndEnqueued`, log lifecycle privacy-safe, test JVM mirati e full `testDebugUnitTest` verdi, `assembleDebug` e `lintDebug` verdi. Nessun cleanup outbox, nessuna UI, nessun backend/schema/RPC/RLS/trigger/migration; **TASK-071** resta non bloccante.

**Review/Chiusura 2026-04-28 (TASK-070 DONE):** **`TASK-070`** è **`DONE`** dopo review tecnica severa senza fix codice; aggiornamenti solo documentali di chiusura. Verificati diff completo, DAO `listPendingRetryable`, retry head-of-line, outcome/log privacy-safe, no-regression TASK-068, nessun cleanup outbox, nessuna UI, nessun backend/schema/RPC/RLS/trigger/migration. `DefaultInventoryRepositoryTest` mirato, full `testDebugUnitTest` con JBR + `--no-daemon`, `assembleDebug`, `lintDebug` e `git diff --check` verdi; i comandi Gradle senza `JAVA_HOME` non sono eseguibili in questa shell perché manca Java nel PATH. Al momento della chiusura TASK-070, **TASK-071** era separato/non bloccante; vedi nota successiva per chiusura `DONE`.

**Review/Chiusura 2026-04-28 (TASK-071 DONE):** **`TASK-071`** è **`DONE`** dopo review severa con fix solo documentali/governance. Causa `PayloadValidation` classificata: contratto RPC locale `record_sync_event` rifiuta `p_changed_count > 1000`, mentre Android può inviare eventi compatti massivi con `entity_ids` vuoto e `changed_count` reale oltre 1000. Classificazione finale: `contract_mismatch`, `backend_fix_needed`, `documentation_drift`; residuo `schema_drift_da_chiarire` solo perché lo schema/live corrente non è stato ricontrollato. Nessun deploy live, nessuna modifica SQL/RPC/RLS/trigger/migration, nessun cleanup outbox/`sync_events`, nessuna modifica Kotlin. Follow-up consigliato: TASK-072 backend separato.
...
| Campo               | Valore                                           |
|---------------------|--------------------------------------------------|
| Stato progetto       | **IDLE — lane release train Android completata** |
| Task attivo          | **Nessuno — TASK-142 DONE; TASK-141 preservato e sospeso** |
| Fase task attivo     | **N/A** |
...
| Prossimo passo operativo | Nessun nuovo task: PR/CI/merge TASK-142 nel train gia autorizzato; TASK-141 resta preservato e sospeso. |
| Ultimo aggiornamento | 2026-08-21 — **TASK-142 DONE / REVIEW APPROVED**: P0=P1=P2=P3=0; three-way conflict, paging summary, source session-scoped, immagini/import/empty state/semantics corretti; test mirati, full JVM, assemble/lint e compile androidTest verdi; device assente classificato pending. |
...
**Avvio TASK-142 — 2026-08-21:** coordination key
`MOBILE_STOREFRONT_PRODUCT_CONTROL`; TASK-141 e sospeso, non chiuso e non
riscritto. Baseline `origin/main`
`0406264c7299766b05419f306c320032e427ca2b`, branch linked-worktree
`codex/mobile-storefront-product-control-android-20260821`, file task
`docs/TASKS/TASK-142-mobile-storefront-product-control-android.md`. Il planning
unico resta Client TASK-046; backend Admin/Supabase e gia integrato e validato
su staging prima dell'avvio consumer Android.

**Closeout TASK-142 — 2026-08-21:** review indipendente, un batch FIX e unico
micro-ciclo aggiuntivo ammesso per P1/P2 riproducibili conclusi con
`APPROVED`; severita finali P0=P1=P2=P3=0. Full JVM, assemble, lint, compile
androidTest e test mirati verdi; device fisico assente classificato pending.
Il prompt USER_APPROVER del release train copre `DONE` e il merge normale dopo
CI exact-SHA verde; nessun rollout production eseguito.
...
boundary manuali, import, repository, sync inbound/outbound ed export. Fixture
golden Admin con SHA-256
`139d63eedea47b54bb63a9289bef5fc6f7372668f209aac7753b586da7ccd9f8`.
Worktree pulito da `origin/main` `ca0a58d...` sul branch
`codex/catalog-text-integrity-android-20260727`; checkout dirty originario
preservato. Staging-only; production e Win7POS non modificati. File task:
`docs/TASKS/TASK-140-catalog-text-integrity-android.md`; evidence:
`docs/TASKS/evidence/TASK-140/README.md`.

**Handoff TASK-140 — 2026-07-27:** lane Android in `REVIEW`. Fixture Admin
byte-identico con SHA-256
`139d63eedea47b54bb63a9289bef5fc6f7372668f209aac7753b586da7ccd9f8`;
policy Kotlin pura e boundary manual/import/full DB/repository/fingerprint/sync
inbound/outbound/recovery completati. Warning Import Analysis e messaggi
IT/EN/ES/ZH inclusi. Gate finali: full JVM `860` totali (`855` eseguiti +
`5` skip intenzionali), `0` failure/error; build app/test, lint, diff-check e
connected API 35 `3/3` verdi. Smoke UI completo picker Database/manual dialog
e staging cross-platform restano esplicitamente pendenti. Nessun stage,
commit, push, deploy o write remoto; production e Win7POS `NOT_MODIFIED`.

**Handoff post-FIX TASK-140 — 2026-07-27:** i finding review R-01–R-05
sono `FIXED_VERIFIED` localmente e la lane torna in `REVIEW`, mai `DONE`.
Collisioni raw→canonical strict sono bloccate in analyzer/full DB/apply;
`RowImportError` non conserva né esporta raw proibito; il repair pre-push
include transazionalmente i campi testuali nella PATCH; supplier/category
mantengono diagnostici tipizzati bounded. Gate finali: full JVM `871`
(`866` eseguiti + `5` skip intenzionali), `0` failure/error; assemble app e
androidTest, lint e connected API 35 `7/7` PASS. Il device copre componenti
production Import Analysis/Database header/dialog e boundary XLSX/Room/sync;
resta dichiarato il gap dell’Activity Result picker e del full stack auth.
Nessuno stage, commit, push, deploy o write remoto.

**Handoff post-rereview TASK-140 — 2026-07-27:** il P1 residuo nel
last-row-wins è corretto tracciando ogni `itemNumber` raw→canonical durante la
prima scansione dell’`ImportAnalyzer`; una coppia convergente sullo stesso
barcode conserva un errore redatto sulla riga coinvolta e blocca preview/apply.
Il test copre analyzer diretto e workbook full database reale. A-14 è ora
coperto da uno smoke screen-level che monta `ImportAnalysisScreen`, osserva il
callback di conferma, monta `DatabaseScreen` con Room in-memory/ViewModel
controllati, apre l’`EditProductDialog` completo da una riga production e
verifica la persistenza canonica. Gate finali: full JVM `873` (`868` eseguiti
+ `5` skip intenzionali), `0` failure/error; assemble app/androidTest, lint,
connected API 35 `8/8` e `git diff --check` PASS. Lane ancora `REVIEW`, mai
`DONE`; nessuno stage, commit, push, deploy o write remoto.

**Chiusura TASK-140 — 2026-07-27:** dopo i merge normali, l'acceptance
cross-platform staging, il paging Win7POS-equivalente e il cleanup a residuo
zero, l'utente ha confermato esplicitamente la chiusura. TASK-140 passa a
`DONE / USER_CONFIRMED_CLOSURE`; progetto `IDLE`. Nessuna modifica runtime,
production o Win7POS nella transizione documentale finale.

**Handoff TASK-139 — 2026-07-18:** follow-up production hardening senza
reimplementare TASK-137/TASK-138. Questa lane consuma il contratto canonico
...
allora non chiudibile `DONE` senza conferma utente; conferma poi ricevuta nel
final review closeout del `2026-07-25`.

**Final review closeout TASK-139 — 2026-07-25:** la conferma utente richiesta è
stata ricevuta. Stato `DONE`; SHA codice
`28f45bbfb34fd5771de8e964470d5de597588a11` verificata su `main` e antenata del
tip documentale. CI esistente `30174297767` `SUCCESS`: assemble, lint e `834`
test totali (`829` eseguiti, `5` skipped intenzionali, `0` failure/error).
P0/P1/P2 aperti `0`. Device/camera fisici restano evidence esterna opzionale e
non bloccante; nessuna produzione o migrazione è stata mutata.

**Addendum P0 TASK-139 — 2026-07-19:** restore Android valida una sessione
persistita con un refresh prima di `SignedIn`, logout esplicitamente locale e
binding account/shop spostato nello stesso Room DB con migration additiva
`20→21`. Mismatch/unbound dirty bloccano catalogo, history, manual sync,
realtime e immagini senza reset; Options espone review/conteggi, discard unbound
con la conferma esistente e replace mismatch come singolo tap distruttivo nella
dialog Material 3 a due azioni. Il checkpoint host
iniziale `146/146` è stato superseduto dal rerun finale: suite P0 mirate
`101/101`, full JVM `674` totali (`669 PASS`, `5` skip solo
opzionali/live), assemble app/test e lint verdi. Sul simulatore Android
autenticato l'APK finale è stato installato in-place senza clear/reset: Room
`user_version=21`, inode e conteggi business preservati, binding account/shop
presente, immagine prodotto reale visibile online e da disk cache dopo cold
launch offline. Alla riconnessione è avvenuto un solo retry del contesto shop,
poi stato `READY`; outbox `0`, nessun upload/replace/remove o duplicato.

**Addendum UX mismatch TASK-139 — 2026-07-19:** per autorizzazione utente la
lane Android non è più `review-only` esclusivamente per questa patch UI. Account
e shop mismatch usano una sola dialog Material 3 con `Mantieni dati locali` e
`Sostituisci con dati cloud`; keep/back/dismiss sono no-op business, la seconda
conferma è rimossa, il replace resta disabilitato senza auth/shop/snapshot
verificati e riusa coordinator/transazione owner-safe esistenti. Presentazione
automatica al massimo una volta per fingerprint mismatch, riapertura manuale da
`Rivedi`. L'audit P1 ha inoltre registrato upload/remove immagini nella lease
TASK-126 esistente, con stop pre/post per intent, PUT/retry, finalize, remove e
...
governa un solo writer Android. Sequenza obbligatoria: baseline invariata,
backend locale multi-ruolo verificato dal coordinatore, poi patch Kotlin mirate,
test/emulator serializzati ed handoff `REVIEW`, mai `DONE`. File task:
`docs/TASKS/TASK-138-product-images-runtime-completion-android.md`; evidence:
`docs/TASKS/evidence/TASK-138/README.md`. TASK-137 resta una voce precedente in
`REVIEW_WITH_BLOCKERS`, non un secondo task attivo.

**Handoff TASK-138 — 2026-07-18:** lane Android portata in `REVIEW`, mai
`DONE`. Implementati main editor fit, batch/dedup/coalescing con chunk `<=100`
e massimo 4 download, lifecycle visible-only con memoria lista limitata,
purge cache su cambio scope, validazione decode prima della cache, progress e
cancel upload. Suite JVM completa, build e lint verdi; emulator, screenshot e
parity Supabase live sono `NOT_RUN_SCOPE` per direttiva del coordinatore.

**Chiusura TASK-138 — 2026-07-18:** l'utente ha richiesto esplicitamente la
review completa e la chiusura. Parity bidirezionale Android/iOS/Admin, remove e
cleanup sono `PASS`; gate finali JVM/build/lint/visual verdi. Stato `DONE`, fase
`DONE_RECONCILED`, verdict `RELEASE_READY_WITH_MEASURED_GATES`. Device fisico e
staging autenticato restano note esterne dichiarate.

**Apertura TASK-137 — 2026-07-16:** il prossimo ID globale libero viene usato
per la feature Product Catalog Images coordinata dal contratto canonico Admin
Web. Android e uno dei tre client in scope; Win7POS e escluso. La lane Kotlin
parte soltanto dopo schema/API backend verificati localmente. Vietati blob o
URL firmati in Room/outbox, service role client-side, nuove dipendenze non
necessarie, production, commit e push. File task:
...
**Nota workflow 2026-04-28 (chiusura TASK-070):** **TASK-070** è **`DONE`** dopo review tecnica; nessun task attivo in quel momento. **TASK-068** resta `PARTIAL`; **TASK-071** era backend separato/non bloccante ed è chiuso nella nota successiva.

**Nota workflow 2026-04-28 (chiusura TASK-071):** **TASK-071** è **`DONE`** dopo review severa; nessun task attivo. La causa `PayloadValidation` è documentata come mismatch tra cap RPC locale `p_changed_count <= 1000` e payload Android compatto con `changed_count` massivo. Fix backend solo proposto come TASK-072; non implementato.
...
| Campo | Valore |
|-------|--------|
| Stato | **`DONE`** (2026-04-28 — review tecnica completata senza fix codice) |
| Priorità | `ALTA` |
...
| Descrizione | Chiuso dopo review: `retrySyncEventOutbox` usa `listPendingRetryable(ownerUserId, maxAttempts, limit)` e raggiunge righe `attemptCount<MAX_ATTEMPTS` anche con head FIFO esaurito; `listPending` resta invariata. Outcome espliciti e log/metriche privacy-safe confermati; test JVM mirati + full `testDebugUnitTest` con JBR `--no-daemon`, `assembleDebug`, `lintDebug`, `git diff --check` verdi. **Nessun** reset/delete/truncate outbox, nessuna UI, nessuna modifica schema/RPC/backend. |

### TASK-071 — Verifica contratto RPC `record_sync_event` / cause `PayloadValidation`
| Campo | Valore |
|-------|--------|
| Stato | **`DONE`** (2026-04-28 — review PASS CON FIX DOCUMENTALI) |
| Priorità | `ALTA` |
...
| Descrizione | Chiuso dopo review diagnostica/documentale: il contratto RPC locale `record_sync_event` rifiuta `p_changed_count > 1000`, mentre Android puo' produrre eventi compatti massivi con `entity_ids` vuoto e `changed_count` reale oltre 1000. Classificazione finale: `contract_mismatch`, `backend_fix_needed`, `documentation_drift`; residuo `schema_drift_da_chiarire` solo per live corrente non ricontrollato. Nessun deploy live, nessuna modifica SQL/RPC/RLS/trigger/migration, nessun cleanup outbox/`sync_events`, nessuna modifica Kotlin. Follow-up consigliato: TASK-072 backend separato. |

### TASK-073 — Filtro conservativo righe riepilogo/footer Excel ordini (合计 / 总数+总价)
| Campo | Valore |
|-------|--------|
| Stato | **`DONE`** (2026-05-17 — review end-to-end APPROVED con fix applicati) |
| Priorità | `ALTA` |
| Area | Import / Excel / parsing — `ExcelUtils.kt`, anteprima PreGenerate, Generated, ImportAnalysis |
| Dipendenze | **TASK-043** `DONE` (stesso perimetro `isSummaryRow` — questo task **rafforza** dove i casi reali passano ancora); **TASK-005** baseline test; **TASK-072** non prerequisito (backend riservato) |
| File task | `docs/TASKS/TASK-073-excel-footer-summary-row-filter.md` |
| Descrizione | Chiuso dopo review end-to-end: parser footer/totali `总数`/`总价`/`合计` resta nel punto comune `analyzeRowsDetailed`; total quantity summary Android+iOS deriva da `quantity` senza cambiare schema; Generated/order detail usa layout compatto 2x2. Fix review applicati: rimosso alias parser fuori scope, memoizzati riepiloghi Compose, allineato parsing quantità CL iOS e test aggiunti. Check verdi: Android `ExcelUtils`, `OrderQuantitySummary`, `ExcelViewModel`, `History`, full `testDebugUnitTest`, `assembleDebug`, `lintDebug`, `git diff --check`; iOS `xcodebuild -list`, `plutil -lint`, `OrderQuantitySummaryTests`, build simulatore, `git diff --check`; file reali in `~/Downloads` verificati. **TASK-072** resta slot backend RPC separato/non toccato. |

### TASK-074 — Warning cleanup Android con migrazione toolchain
| Campo | Valore |
|-------|--------|
| Stato | **`DONE`** (2026-05-27 — review APPROVED, verifiche verdi) |
| Priorita | `ALTA` |
| Area | Build / Toolchain / Lint / Risorse |
| Dipendenze | **TASK-029** `DONE` (rinvio built-in Kotlin/new DSL); **TASK-012** `DONE` |
| File task | `docs/TASKS/TASK-074-warning-cleanup-toolchain-android.md` |
| Descrizione | Chiuso dopo review repo-grounded APPROVED: migrazione AGP built-in Kotlin/new DSL, Gradle 9.5.1, cleanup Compose/stringhe/risorse low-risk; lint `169 -> 99`, build/lint/test/diff check verdi; Supabase/Ktor, target SDK, POI jar trust manager, plurals e stringhe unused rinviati/documentati. |

### TASK-075 — Warning cleanup Android residui low-risk
| Campo | Valore |
|-------|--------|
| Stato | **`REVIEW`** (2026-05-27 — execution completata, verifiche verdi) |
| Priorita | `ALTA` |
| Area | Lint / Compose / Risorse / Localizzazione |
| Dipendenze | **TASK-074** `DONE` |
| File task | `docs/TASKS/TASK-075-warning-cleanup-android-residui-low-risk.md` |
| Descrizione | DONE dopo review finale: la nuova API Compose swipe ha creato regressione reale, quindi `confirmValueChange` e' stato ripristinato solo nei due call-site interessati con `@Suppress("DEPRECATION")` locale; `PluralsCandidate` convertiti a plurals, stringhe `UnusedResources` rimosse se staticamente sicure, audit supplementare ha rimosso 16 valori lingua legacy nascosti da `tools:ignore`, Supabase Kotlin `3.6.0` e Ktor `3.5.0` mantenuti su conferma utente; `lintDebug` da `99` a `3` warning, build/lint/test verdi e smoke emulator PASS. Restano fuori scope `OldTargetApi` e POI trust manager esterno. |
