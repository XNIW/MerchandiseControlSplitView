...
**Nota 2026-04-25 (stato corrente):** **TASK-058** — **`PLANNING`** — `DatabaseScreen`: refresh locale dopo modifica prodotto (lista paginata non stale) e scroll lista stabile; file `docs/TASKS/TASK-058-database-screen-refresh-locale-scroll-stabile.md`. **Prossimo passo operativo:** approvazione utente per **EXECUTION**. **TASK-057** — **`DONE`** (2026-04-25).
...
| Prossimo passo operativo | **Approvazione utente** per passare **TASK-058** da **PLANNING** a **EXECUTION** (fix refresh locale + scroll; vedi `docs/TASKS/TASK-058-database-screen-refresh-locale-scroll-stabile.md`). |
| Ultimo aggiornamento | 2026-04-25 — creato **TASK-058** `PLANNING` (`DatabaseScreen` / Paging + override VM); `MASTER-PLAN` e backlog riallineati; **TASK-057** resta `DONE` (2026-04-25). |
...
| Descrizione | Dopo salvataggio da **Modifica prodotto**, la card in lista e il dialog al rientro mostrano subito i prezzi coerenti con Room/storico; niente snapshot Paging stale. Scroll lista non deve saltare in cima per un singolo update. Meccanismo: query puntuale `ProductWithDetails` by id + override nel VM, merge in lista — **nessun** cambio schema Room, **nessun** remove Paging 3, **nessun** redesign schermata. Baseline test TASK-004 se si toccano repository/VM. Verifica pull catalogo: documentata, non ampliata oltre perimetro. |
