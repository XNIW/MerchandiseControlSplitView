...
| Campo               | Valore                                           |
|---------------------|--------------------------------------------------|
| Task attivo          | **Nessun task attivo** — **TASK-061** chiuso in `DONE`; **TASK-060** resta sospeso / `BLOCKED` per decisione esplicita utente del 2026-04-26, senza chiusura `DONE`. |
| Fase task attivo     | **N/A** — nessun task in `PLANNING` / `EXECUTION` / `REVIEW`; prossimo task da scegliere esplicitamente. |
...
| Campo       | Valore                                                  |
|-------------|---------------------------------------------------------|
| Stato       | `PARTIAL` (audit/verifiche live e chiusura audit Supabase non completamente concluse; **TASK-059** `DONE`; **TASK-060** sospeso / `BLOCKED`; **TASK-061** `DONE`; follow-up residui in **TASK-062**–**TASK-063** o decisione futura) |
| Priorità    | `ALTA`                                                  |
...
> **Contesto:** follow-up a **TASK-055** ( **`PARTIAL`** ). **TASK-059** è **`DONE`**. **TASK-060** è **sospeso / `BLOCKED`** per decisione esplicita utente del 2026-04-26, con review non chiusa; **TASK-061** è **`DONE`** dal 2026-04-26 su conferma utente. Non c'e' nessun task attivo; il prossimo passo operativo e' scegliere esplicitamente **TASK-062** o **TASK-063**. **TASK-062**–**TASK-063** restano in `BACKLOG` con descrizioni sotto.
...
| Dipendenze | **TASK-055** `PARTIAL` (chiude/integra le verifiche `PARTIAL` rimaste) |
| Descrizione | Scenari minimi: device A e B stesso account Supabase; (1) A modifica prodotto/prezzo → B riceve update remoto **senza** scroll jump; (2) A aggiunge prodotto → B riceve; (3) A elimina con tombstone → B riceve; (4) offline → modifica locale → online → auto-push; (5) `sync_events` non disponibile/gap → UI propone full sync. **Nessun codice** salvo bug emersi. **Output richiesto:** matrice risultati; elenco bug; follow-up tecnici in task separati. **Nota:** questo master-plan **non** dichiara smoke/build eseguiti. |
