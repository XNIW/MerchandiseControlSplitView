...
**Nota 2026-04-26 (TASK-065/TASK-064/TASK-063 post-fix):** **TASK-065** e' **`DONE`**: root cause client-side isolata (`decodeSingle()` list-only vs RPC `record_sync_event` che ritorna object/row) e fix Android minimo applicato. **TASK-064** e' **`DONE`**: B1-B9 post-fix verdi/con limite documentato, outbox A/B `0`, nessun nuovo `PayloadValidation` su S2 modifica+rollback. **TASK-060** e' **`DONE`**: S2 post-fix verifica update remoto puntuale in `DatabaseScreen` senza search/scroll jump. **TASK-063** resta **`BLOCKED`** in modalita `ACCEPTABLE` (OnePlus IN2013 + Medium Phone API 35, non `FULL`): S1/S2 PASS, S3-S6 non eseguiti. **TASK-055** resta **`PARTIAL`**. Nessuna migration live, nessun `supabase db push`, nessuna modifica DDL/RPC/RLS/publication Supabase remota.
...
| Campo               | Valore                                           |
|---------------------|--------------------------------------------------|
| Task attivo          | **Nessun task attivo da eseguire automaticamente**: **TASK-065** `DONE`, **TASK-064** `DONE`, **TASK-063** `BLOCKED`, **TASK-060** `DONE`, **TASK-055** `PARTIAL`. |
| Fase task attivo     | **Blocco residuo TASK-063** — S3-S6 smoke live non eseguiti; non dichiarare `FULL` con OnePlus + emulator. |
...
| Milestone            | **TASK-065** **`DONE`** (2026-04-26 — fix client-side `record_sync_event` response handling; build/lint/test/live S2 verdi). **TASK-064** **`DONE`** (2026-04-26 — baseline B1-B9 verde/con limite documentato; outbox A/B `0`). **TASK-060** **`DONE`** (2026-04-26 — update remoto puntuale DatabaseScreen verificato da S2 post-fix). **TASK-063** **`BLOCKED`** (2026-04-26 — `ACCEPTABLE`: S1/S2 PASS; S3-S6 non eseguiti; non `FULL`, non `DONE`). **TASK-062**/**TASK-061**/**TASK-059** `DONE`; **TASK-055** `PARTIAL`. |
| Prossimo passo operativo | Planner/reviewer: decidere se completare TASK-063 S3-S6 in `ACCEPTABLE` con dataset/rollback controllato o attendere due device reali per `FULL`. TASK-055 resta `PARTIAL` finche' lo smoke core non e' completo. |
| Ultimo aggiornamento | 2026-04-26 — **TASK-065** `DONE`; **TASK-064** `DONE`; **TASK-060** `DONE`; **TASK-063** `BLOCKED`; **TASK-055** `PARTIAL`; nessuna modifica Supabase live. |
...
| Campo       | Valore                                                  |
|-------------|---------------------------------------------------------|
| Stato       | `PARTIAL` (audit/verifiche live e chiusura audit Supabase non completamente concluse; **TASK-059**/**TASK-060**/**TASK-061**/**TASK-062**/**TASK-064**/**TASK-065** `DONE`; follow-up residui in **TASK-063** S3-S6 o decisione futura) |
| Priorità    | `ALTA`                                                  |
...
> **Contesto:** follow-up a **TASK-055** ( **`PARTIAL`** ). **TASK-059** è **`DONE`**. **TASK-060** è **`DONE`**. **TASK-061** è **`DONE`**. **TASK-062** è **`DONE`**. **TASK-063** è **`BLOCKED`** per S3-S6 non eseguiti. **TASK-064** è **`DONE`**. **TASK-065** è **`DONE`**.
...
| Campo | Valore |
|-------|--------|
| Stato | **`BLOCKED`** (2026-04-26 — post TASK-065: S1/S2 PASS in `ACCEPTABLE` senza nuova outbox; S3-S6 non eseguiti; non `FULL`, non `DONE`) |
| Priorità | `ALTA` |
| Area | QA manuale / Supabase live / multi-device |
| File task | `docs/TASKS/TASK-063-smoke-live-a-b-sync-android-supabase.md` |
| Dipendenze | **TASK-055** `PARTIAL`; **TASK-059**/**TASK-060**/**TASK-061**/**TASK-062**/**TASK-064**/**TASK-065** `DONE` |
...
| Dipendenze | **TASK-063** `BLOCKED`; **TASK-062**/**TASK-061**/**TASK-060**/**TASK-065** `DONE`; **TASK-055** `PARTIAL` |
...
| Dipendenze | **TASK-064** `DONE`; **TASK-063** `BLOCKED`; **TASK-060** `DONE`; **TASK-055** `PARTIAL` |
