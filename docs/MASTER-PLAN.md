...
| Campo               | Valore                                           |
|---------------------|--------------------------------------------------|
| Task attivo          | **TASK-060** — `REVIEW` (**bloccante**: refresh puntuale `DatabaseScreen` post-pull) — *unico task attivo*. **TASK-059** chiuso `DONE` (2026-04-26). **TASK-055** resta `PARTIAL`; **TASK-058** / **TASK-057** restano `DONE` (2026-04-25). |
| Fase task attivo     | **`REVIEW`** (TASK-060) — bloccante fino a verdetto planner / `DONE` o sospensione esplicita. |
...
| Campo       | Valore                                                  |
|-------------|---------------------------------------------------------|
| Stato       | `PARTIAL` (audit/verifiche live e chiusura audit Supabase non completamente concluse; **TASK-059** `DONE`; **TASK-060** in `REVIEW` **bloccante**; follow-up residui in **TASK-061**–**TASK-063**) |
| Priorità    | `ALTA`                                                  |
...
> **Contesto:** follow-up a **TASK-055** ( **`PARTIAL`** ). **TASK-059** è **`DONE`**. **TASK-060** è in **`REVIEW`** (unico attivo, **bloccante** fino a chiusura review) con file dedicato; **TASK-061** ha **piano inizializzato** in `docs/TASKS/TASK-061-hardening-sync-events-fallback-full-sync.md` (`PLANNING` nel file task, **`BACKLOG`** nel master finché non attivato); **TASK-062**–**TASK-063** restano in `BACKLOG` con descrizioni sotto.
...
| Campo | Valore |
|-------|--------|
| Stato | `REVIEW` (2026-04-25 — post-`EXECUTION`; **bloccante** fino a verdetto review planner / conferma `DONE` o ciclo `FIX`) |
| Priorità | `ALTA` |
...
| Campo | Valore |
|-------|--------|
| Stato | `BACKLOG` (piano inizializzato in file task `PLANNING`; **non** promuovere a task attivo finché **TASK-060** è `REVIEW` bloccante senza transizione esplicita) |
...
| Descrizione | Rendere provati e comunicati i casi limite: eventi troppo grandi; gap watermark; eventi senza ID utilizzabili; schema/RLS/RPC non disponibile; outbox pending; `manualFullSyncRequired`. **Esito utente:** quando la quick sync non basta, l’app lo comunica e propone una sync completa. **Escluso:** redesign completo della sync; nessuna architettura cloud-first nuova. |
