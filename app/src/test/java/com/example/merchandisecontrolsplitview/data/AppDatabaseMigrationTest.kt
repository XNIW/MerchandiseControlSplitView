...
class AppDatabaseMigrationTest {
 ...
 VALUES (7, 1, 'PURCHASE', 9.99, '2026-04-17 10:00:00', 'MANUAL', '2026-04-17 10:00:00')"""
    ...
    @Test
    fun `migration 12 to 13 adds pending_catalog_tombstones`() = runTest {
        val dbName = "task019-migrate-12-to-13.db"
        createLegacyDatabase(dbName, version = 12) { }

        val migrated = openSupportMigratedDatabase(dbName, targetVersion = 13) { database, oldVersion, newVersion ->
            assertEquals(12, oldVersion)
            assertEquals(13, newVersion)
            AppDatabase.MIGRATION_12_13.migrate(database)
        }
        assertEquals("13", querySingleValue(migrated, "PRAGMA user_version"))
        assertTrue(tableExists(migrated, "pending_catalog_tombstones"))
        assertEquals(0, queryCount(migrated, "PRAGMA foreign_key_check"))
        migrated.close()
    }

    @Test
    fun `migration 13 to 14 adds displayName to history entries`() = runTest {
        val dbName = "task040-migrate-13-to-14.db"
        createLegacyDatabase(dbName, version = 13) { db ->
            db.execSQL(
                """
                CREATE TABLE IF NOT EXISTS history_entries(
                    uid INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
            ...
            db.execSQL(
                """
                INSERT INTO history_entries(
                    uid, id, timestamp, data, editable, complete, supplier, category,
                    wasExported, syncStatus, orderTotal, paymentTotal, missingItems,
                    totalItems, isManualEntry
                ) VALUES (
                    1, 'legacy.xlsx', '2026-04-22 10:00:00', '[["barcode"]]',
                    '[["",""]]', '[false]', '', '', 0, 'NOT_ATTEMPTED', 0, 0, 0, 0, 0
                )
                """.trimIndent()
            )
        }

        val migrated = openSupportMigratedDatabase(dbName, targetVersion = 14) { database, oldVersion, newVersion ->
        ...
        val hasDisplayName = migrated.query("PRAGMA table_info(`history_entries`)").use { cursor ->
            var found = false
            while (cursor.moveToNext()) {
                found = found || cursor.getString(cursor.getColumnIndexOrThrow("name")) == "displayName"
            }
            found
        }
        assertTrue(hasDisplayName)
        assertEquals("", querySingleValue(migrated, "SELECT displayName FROM history_entries WHERE uid = 1"))
        migrated.close()
    }
    ...
    @Test
    fun `migration 18 to 19 adds sync event apply status table`() = runTest {
        val dbName = "task114-migration-18-19-sync-event-apply-status.db"
        createLegacyDatabase(dbName, version = 18) { }

        val migrated = openSupportMigratedDatabase(dbName, targetVersion = 19) { database, oldVersion, newVersion ->
            assertEquals(18, oldVersion)
            assertEquals(19, newVersion)
            AppDatabase.MIGRATION_18_19.migrate(database)
        }

        assertEquals("19", querySingleValue(migrated, "PRAGMA user_version"))
        assertTrue(tableExists(migrated, "sync_event_apply_status"))
        assertTrue(indexExists(migrated, "index_sync_event_apply_status_owner_user_id_store_scope_status"))
        assertTrue(indexExists(migrated, "index_sync_event_apply_status_owner_user_id_store_scope_next_retry_at_ms"))
        assertTrue(columnExists(migrated, "sync_event_apply_status", "attempt_count"))
        assertTrue(columnExists(migrated, "sync_event_apply_status", "next_retry_at_ms"))
        migrated.close()
    }
