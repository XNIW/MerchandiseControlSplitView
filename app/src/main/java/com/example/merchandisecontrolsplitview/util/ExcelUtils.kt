...
internal val KNOWN_EXCEL_HEADER_ALIASES: Map<String, List<String>> = mapOf(
    ...
    "oldPurchasePrice" to listOf("oldpurchaseprice", "prevpurchase", "prev purchase", "prezzovecchioacquisto", "prezzoprecedenteacquisto", "acquistoprec","previouspurchaseprice","Prezzo vecchio acquisto", "旧进价", "Precio de compra anterior", "Old purchase price", "Purchase (Old)", "Acquisto (Vecchio)", "Compra (Antiguo)", "进价（旧）"),
    "oldRetailPrice" to listOf("oldretailprice", "prevretail", "prev retail", "prezzovecchiovendita", "prezzoprecedentevendita", "venditaprec", "previousretailprice", "Prezzo vecchio vendita", "旧零售价", "Precio de venta anterior", "Old retail price", "Retail (Old)", "Vendita (Vecchio)", "Venta (Antiguo)", "售价（旧）")
)
...
internal fun analyzeRowsDetailed(
    ...
    header.forEachIndexed { col, key ->
        if (key == "itemNumber" && headerSource.getOrNull(col) == "pattern") {
            val raw = rawHeader.getOrNull(col).orEmpty()
            val aliases = KNOWN_EXCEL_HEADER_ALIASES["itemNumber"].orEmpty()
            if (aliases.any { alias -> headerMatchesAlias(raw, alias) }) {
                headerSource[col] = "alias"
            }
        }
    }
